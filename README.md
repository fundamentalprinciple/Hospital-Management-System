If you're unfamiliar with python development, follow these step to run my webapp:
1) Clone the repo
2) Make a python3 virtual environment (run this command for ubuntu: python3 -m venv [name of the venv])
3) Activate the venv (run: source [name of the venv]/bin/activate)
4) Install the python dependancies (run: pip install -r requirements.txt )
5) Set up your own environment variables (run:
                                               echo "SECRET_KEY = '[ make a difficult string yourself ]'
                                                     SECURITY_PASSWORD_SALT = '[ make a difficult string yourself again ]'
                                                     Admin = '[ password for admin, admin is pre defined, you are defining admin right now! ]'")
6) Run: py main.py
7) In a browser, visit: http://localhost:8000
